...
// Fetch user's orders
$orders = $mysqli->query("SELECT * FROM orders WHERE user_id = $user_id ORDER BY created_at DESC")->fetch_all(MYSQLI_ASSOC);
?>
...
<head>
<meta charset="UTF-8">
<title>My Orders — Lumineux Opticals</title>
...
.page-header h1{font-family:'Playfair Display',serif;font-size:2.6rem;font-weight:600;margin-bottom:12px;}
...
<div class="page-wrap">
    <div class="page-header">
        ...
        <p style="color:black; font-size: 20px;">Review your past purchases and their details.</p>
    </div>
        ...
        <?php if(!empty($orders)): ?>
            <?php foreach($orders as $order): ?>
                <?php
                    ...
                    $items_res = $mysqli->query("SELECT oi.*, p.name, p.image_path, p.price 
                                                FROM order_items oi 
                ...
                <div class="order-card">
                    ...
                    <div style="text-align:center;">
                        <a href="order-details.php?id=<?= $order_id ?>" class="view-btn">View Details</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <p style="text-align:center;color:#111;font-size:20px;">You have no orders yet.</p>
        <?php endif; ?>
    </div>
</div>
<!-- FOOTER -->
